...
**TCP reassembly / `SessionTracker`:** opt-in, bounded FIFO eviction, real overlap handling (`TcpOverlapPolicy::Reject`/`FirstWins`/`LastWins`, default `Reject`). `RST` tears the flow down; a `SYN` on an already-open direction restarts the whole flow (both directions, new generation) instead of getting silently appended to the old stream. `SessionTracker` clears its own probe state on the same generation change, not just on `RST`. `TcpStreamReassembler`/`QuicStreamReassembler`/`SessionTracker`/`IpFragmentReassembler` also cap total buffered bytes across every flow/stream/probe/datagram combined (`with_max_total_buffered_bytes`/`with_max_total_probe_bytes`/`with_max_total_bytes`, 64 MiB default), not just per-entry.

**New, not fully wired in yet:** `ParseError`/`ParseErrorKind`/`Layer` add offset/protocol context that `LayerError` doesn't carry, but most parsers still return plain `LayerError`. `ProbeResult<T>` separates "not this protocol" from "truncated" (with `needed`/`available` byte counts) from "malformed" (DNP3, HTTP, TLS ClientHello, DNS, SSH banner, SMB1, SMB2, BGP, MQTT, FTP, NNTP, IMAP, LDAP, SMTP, and Modbus probes use it; the rest still return `Option<T>`). The ~39 application fields moved behind `application: Option<Box<ApplicationLayers>>`. Read them through the per-protocol accessors, `parsed.dns()` and so on, or `application_layer()` for the first one that matched. Keeps `ParsedPacket` at 856 bytes instead of 2736, which a transport-only parse was paying for.
